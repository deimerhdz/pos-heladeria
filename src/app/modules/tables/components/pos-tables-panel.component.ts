  ...
  template: `
    <div class="w-full sm:w-[300px] shrink-0 flex flex-col border-r border-gray-200 min-h-0 bg-white">
      <div class="p-4 border-b border-gray-100 space-y-3 shrink-0">
        <h4 class="text-sm font-bold text-gray-900">Mesas</h4>
        <input
          #searchInput
          type="text"
          [value]="store.search()"
      ...
      <div class="flex-1 overflow-y-auto p-3 space-y-2.5">
        @for (t of store.tablesView(); track t.id) {
          <button
            ...
            class="w-full text-left bg-white rounded-xl border p-3 space-y-2 transition-colors hover:border-indigo-300"
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <span class="text-[11px] font-semibold px-2 py-0.5 rounded-full" [class]="t.chipClass">{{ t.statusLabel }}</span>
            </div>
            ...
            <div class="flex items-center justify-between">
              <span class="text-base font-bold text-gray-900">{{ t.totalLabel }}</span>
              @if (t.ordersCount > 1) {
                <span class="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{{ t.ordersCount }} pedidos</span>
              }
            </div>
          </button>
        }
        @if (store.noTablesFound()) {
          <p class="text-center text-sm text-gray-400 py-8">Sin resultados</p>
        }
      </div>
    </div>
  `,
